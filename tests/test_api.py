...
class SearchQuestionServiceTest(unittest.TestCase):
    def test_deputes(self):
        ...
        self.assertEqual(api.search_parliamentarians('Cope').nom_de_famille, u'Copé')

    def test_senateurs(self):
        api = CPCApi(ptype='senateur')
        self.assertGreater(len(api.parlementarians()), 1)
        self.assertEqual(api.search_parliamentarians('Larcher').nom_de_famille, u'Larcher')

    def test_2007_2012(self):
        api = CPCApi(legislature='2007-2012')
        self.assertGreater(len(api.parlementarians()), 1)
        self.assertEqual(api.search_parliamentarians('Morano').nom_de_famille, u'Morano')
    ...
    def test_2017_2022(self):
        ...
        self.assertEqual(api.search_parliamentarians('Melenchon').nom_de_famille, u'Mélenchon')
